...
const IsExistRecipe = (property: string, validationOptions?: ValidationOptions) => {
  return function(object: Object,propertyName: string){
    registerDecorator({
      name: 'IsExistRecipe',
      target:  object.constructor,
      ...
      validator:{
        async validate(value: number, args: ValidationArguments):Promise<boolean>{
          try {
            const findRecipe = await RecipeModel.findOne({
             _id: value
            },{_id:1}).lean()

            if(!findRecipe) throw new Error()
            return true;
          } catch (error) {
            return false
          }
...
export { MatchPassword, IsInFitnessGoalList, IsExistExercise, IsExistRecipe};